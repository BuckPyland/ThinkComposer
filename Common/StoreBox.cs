   ...
   public class StoreBox<TValue> : StoreBoxBase
   {
      ...
      public StoreBox(IUniqueElement ReferencesCentralizer)
           : this(ReferencesCentralizer != null && !typeof(TValue).IsValueType
                  ? ReferencesCentralizer.GlobalId : default)
      {
      }
      ...
      {
         var Result = new StoreBox<TValue>(CentralizerId)
         {
            Value = this.Value
         };
      ...
      public override void CentralizeReferencesIn(Guid ReferencesCentralizer)
      {
         ...
         if (ReferencesCentralizer == null || CentralizerId != null)
            return;
         ...
         // CENTRALIZE
         CentralizerId = ReferencesCentralizer;
         ...
         if (!CachedValue.IsEqual(default))
            Value = CachedValue;
         else if (!PreviousValue.IsEqual(default))
            Value = PreviousValue;
      }
      ...
      protected void AssignAccessors()
      {
         ...
         Getter = Register.Item1;
         Setter = Register.Item2;
      }
      ...
      public TValue Value
      {
         get
         {
            if (!ValueIsCached)
            {
               ...
               if (TypeOfValue.IsValueType || CentralizerId == Guid.Empty)
               {
                  CachedValue = Getter(StoredValue);
                  ValueIsCached = true;
               }
               else
               {
                  ...
                  if (StoredValue != null)
                     if (StoreBox.ReferenceCentralizers.ContainsKey(CentralizerId))
                     {
                        var SharedReferences = StoreBox.ReferenceCentralizers[CentralizerId];
                        if (StoredValue.Length != 16)
                           throw new UsageAnomaly("An stored Guid must have a size of 16 bytes.");

                        var ValueKey = new Guid(StoredValue);

                        if (SharedReferences.ContainsKey(ValueKey))
                        {
                           CachedValue = Getter(SharedReferences[ValueKey]);
                           DesignatedObjectsIdentifiers.AddNew(ValueKey, CachedValue);
                        }
                     }
                     else
                     {
                        // Console.WriteLine("FAIL: An stored References-Centralizer code must be associated to existing shared storage.");
                        throw new UsageAnomaly("An stored References-Centralizer code must be associated to existing shared storage.", CentralizerId);
                     }
               }
            }
         ...
         set
         {
            ...
            else
            {
               ...
               if (StoreBox.ReferenceCentralizers.ContainsKey(CentralizerId))
                  SharedReferences = StoreBox.ReferenceCentralizers[CentralizerId];
               else
               {
                  SharedReferences = new Dictionary<Guid, byte[]>();
                  StoreBox.ReferenceCentralizers.Add(CentralizerId, SharedReferences);
               }
               ...
               else
               {
                  ...
                  SharedReferences[ObjectId] = Setter(value);
                  StoredValue = ObjectId.ToByteArray();
               }
            }
      ...
      /// </summary>
      [NonSerialized]
      protected TValue CachedValue = default;
      ...
      {
         return "StoreBox (HC=" + GetHashCode().ToString() + "). Value: " + Value.ToStringAlways();
      }
   }
}